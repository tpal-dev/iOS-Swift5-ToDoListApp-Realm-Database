...
open class GalleryViewController: UIPageViewController, ItemControllerDelegate {
    ...
    fileprivate var sendLayout = ButtonLayout.pinRight(16, 16)
    ...
    fileprivate func configureSelectionButton() {
        ...
        if self.view.subviews.contains(selectionButton) == false {
    ...
    fileprivate func configureSendButton() {
        ...
        sendButton.addTarget(self, action: #selector(GalleryViewController.sendItem), for: .touchUpInside)
        sendButton.alpha = 0
    ...
    open override func viewDidLayoutSubviews() {
        ...
        layoutButton(sendButton, layout: sendLayout)
        layoutHeaderView()
        layoutFooterView()
        layoutScrubber()
    }
    ...
    fileprivate func animateDecorationViews(visible: Bool) {
        ...
        UIView.animate(withDuration: decorationViewsFadeDuration, animations: { [weak self] in
            ...
            self?.sendButton?.alpha = targetAlpha
